...
std::vector<std::unique_ptr<Player>> FileHandler::loadPlayerInfo(void)
{
	...
	while (std::getline(file, line)) {
		...
		if (argv.size() != 3) {
			throw improper_vector_size("Unable to load player class from file");
		}
...
std::vector<std::unique_ptr<Ship>> FileHandler::loadShipInfo(void) {
	...
	while (std::getline(file, line)) {
		...
		if (argv.size() < 5) {
			throw improper_vector_size("Failed to load Ship class from file");
		}
...
std::vector<std::unique_ptr<CruiseMissile>> FileHandler::loadMissileInfo(void) {
	...
	while (std::getline(file, line)) {
		...
		if (argv.size() != 3) {
			throw improper_vector_size("Failed to load Cruise Missile object from file");
		}
